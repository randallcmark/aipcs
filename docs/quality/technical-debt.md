...
| Q012 | Server-controlled fields | Agent-designed schemas can include `id`, `owner_id`, timestamps, and tool provenance even though normal record writes must not override them | Causes validation friction and encourages agents to route around the tool contract if filesystem access is available | Make server-owned fields explicit in schema/tool docs; reject or mark them as non-writable during design/materialisation | Open |
